...
"""
OUTPUT_DIR = exec_dir + '/results'  # Training Result Output Path
PLOT_DIR = exec_dir + '/plots'      # Figure Generate Path
RL_ALGOS = ["RLTCP"]                # RL Algorithms (RLTCP)
TCP_ALGOS = ["TCP_CUBIC"]           # TCP Algorithms (TCP_CUBIC, TCP_NV, PCC, DCTCP)
ALGOS = RL_ALGOS + TCP_ALGOS        # Algorithms (RL + TCP)
TRANSPORT = ["tcp"]                 # Transport Protocols
TF_PATTERNS = [0]                   # Traffic Patterns
RUNS = 1                            # RL Training Times
EPISODES = 100                      # RL Training Episodes
TOPO = "dumbbell"                   # Mininet Network Topology
NETWORK_RATES = [1000]              # Link Bandwidth (Mbps)
TUNE = False                        # Tune Framework set or not 


# Testname generated by Hostname + FolderIndex
...
def dump_config(args, path, pattern):
    ...
    rl_utils.dump_json(path=path, name="bench_config", data=test_config)

# Each round of RL training
def launch_test(algo, results_subdir, transport, rate, topo, pattern, episodes):
    ...
    log.info("%s", cmd)
    ray_inst.train_ray(cmd.split())

# RL overall training process
    ...
    for index in range(args.runs):
        for transport in args.transports:
            ...
            for algo in args.rl_algos:
                launch_test(algo, results_subdir, transport, rate, args.topo, pattern, args.episodes)
        for algo in args.tcp_algos:
            results_subdir = "%s/%s_run%d" % (results_dir, "tcp", index)
            launch_test(algo, results_subdir, "tcp", rate, args.topo, pattern, args.episodes)
...
def init(args):
    ...
    for pattern in args.tf_patterns:
        ...
        for rate in args.network_rates:    
            ...
            plot(results_dir, args.plot_dir, rate, testname)
