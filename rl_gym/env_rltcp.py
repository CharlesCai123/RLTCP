...
from rl_gym.control.rl_bw_control import BandwidthController
from rl_gym.rl_evaluator.rl_sampler import StatsSampler
from rl_gym.rl_evaluator.rl_traffic import TrafficGen
from rl_gym.rl_evaluator.rl_state import StateManager

from rl_gym.rl_evaluator.rl_utils import TopoFactory
from rl_gym.topos.network_manager import NetworkManager
...
DEFAULT_CONF = {
    ...
    # Use dumbbell topology for tests.
    "topo": "dumbbell",
...
class RLTCPEnv(openAIGym):
    ...
    def __init__(self, conf={}):
        ...
        self.short_id = rl_utils.generate_id()
        if self.conf["parallel_envs"]:
            self.conf["topo_conf"]["id"] = self.short_id
        
        # Initialize the topology
        self.topo = TopoFactory.create(self.conf["topo"], self.conf["topo_conf"])

        # Save the configuration we have, id does not matter here
        rl_utils.dump_json(path=self.conf["output_dir"], name="env_config", data=self.conf)
        rl_utils.dump_json(path=self.conf["output_dir"], name="topo_config", data=self.topo.conf)

        # Set the dimensions of the state matrix
        self._set_gym_matrices()
        
        # Set the active traffic matrix
        self._set_traffic_matrix(self.conf["tf_index"], self.conf["input_dir"], self.topo)
        ...
        rl_utils.check_dir(self.conf["output_dir"])
    ...
    def _set_gym_matrices(self):
        ...
        self.tx_rate = rl_utils.shmem_to_nparray(tx_rate, np.float32)
        active_rate = RawArray(ctypes.c_uint32, num_actions)
        self.active_rate = rl_utils.shmem_to_nparray(active_rate, np.float32)
