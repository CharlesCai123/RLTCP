"""
    Module for generating traffic flow in transport layer among virtual servers and routers
"""
import os
import sys
import csv
...
class TrafficGen():
    ...
    def _start_servers(self, hosts, traffic_gen, out_dir):
        ...
        for host in hosts:
            ...
            s_proc = rl_utils.start_process(server_cmd, host, out_file)
    ...
    def _start_controllers(self, hosts, out_dir):
        ...
        if not os.path.isfile(traffic_ctrl):
            ...
            rl_utils.kill_processes(self.service_procs)
            exit(1)
        ...
        for host in hosts:
            ...
            c_proc = rl_utils.start_process(ctrl_cmd, host, out_file)
    ...
    def _start_client(self, traffic_gen, host, out_dir, dst_hosts):
        ...
        if self.transport == "udp":
            traffic_cmd += "-udp "
        t_proc = rl_utils.start_process(traffic_cmd, host, out_file)
    ...
    def _start_pkt_capture_tshark(self, out_dir):
        ...
        dmp_proc = rl_utils.exec_process(dmp_cmd, out_file=dmp_file)
    ...
    def _start_pkt_capture_tcpdump(self, host, out_dir):
        ...
        dmp_proc = rl_utils.start_process(dmp_cmd, host, dmp_file)
        self.service_procs.append(dmp_proc)
    ...
    def _start_generators(self, hosts, input_file, traffic_gen, out_dir):
        ...
        if not os.path.basename(input_file) == "all":
            ...
            if traffic_pattern is None:
                ...
                rl_utils.kill_processes(self.service_procs, use_sigkill=True)
                exit(1)
    ...
    def _stop_traffic(self):
        ...
        rl_utils.kill_processes(self.traffic_procs)
