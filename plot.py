...
PLOT_DIR = os.path.dirname(os.path.abspath(__file__)) + "/plot_tests"
ROOT = "candidates"
...
def plot_lineplot(algos, plt_stats, num_samples, plt_name):
    ...
    for index, metric in enumerate(plt_metrics):
        ...
        if index == 0:
            ...
            plt_legend = False
        ax[index] = sns.lineplot(data=metric_df, ax=ax[index], legend=plt_legend,
...
def get_stats(statistics, stats_dict, metrics):
    ...
    print("adjklasjdklasjdklasjdkla")
...
def plot(data_dir, plot_dir, rate, name):
    ...
    for transport in transports:
        ...
        ### FigureName = folder/[topo]_[transport]_[bandwidth]_[episodes]  ###

        plt_name = "%s/" % (plot_dir)
        plt_name += "%s" % name
        rl_utils.check_dir(plt_name)
        plt_name += "/%s" % topo            
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rate', '-r', dest='rate')
    args = parser.parse_args()

    logging.basicConfig(format="%(levelname)s:%(message)s", level=logging.INFO)
    try:
        plot(args.input_dir, PLOT_DIR, args.rate, 
             os.path.basename(os.path.normpath(args.input_dir)))
        exit(0)
    except Exception as e:
        log.info("%s -- Error has occurred, failed to plot the figure", e)
