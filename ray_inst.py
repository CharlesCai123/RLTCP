"""
    Ray is a distributed Deep-RL framework built by UC Berkeley. We extend our approach by leveraging its APIs
    to train the network with our parameters set as input
"""
...
# set up paths
cwd = os.getcwd()
lib_dir = os.path.dirname(rl_gym.__file__)
...
def get_env(env_config):
    return rl_utils.EnvFactory.create(env_config)
...
def get_gym(env_config):
    ...
    gym.register(
        id='rl-tcp-v0',
        entry_point='rl_gym.env_tcp:RLTCPEnv',
        max_episode_steps=iterations,
    )
    env = gym.make('rl-tcp-v0', conf=env_config)
    return env
...
def set_tuning_parameters(agent, config):
    ...
    if agent.lower() == "ppo":
        ...
        def explore(config):
            config["train_batch_size"] = max(config["train_batch_size"], 2000)  # should be 4 at minimum
...
def get_agent(agent_name):
    if agent_name.lower() == "rnd":
        ...
        return agent_class
    try:
        agent_class = get_trainable_cls(agent_name.upper())
...
def run_ray(config, total_episodes):
    ...
    agent = agent_class(config=config, env="rl_env")
    steps = 0
    episodes = 0
    ...
    log.info("Generator Finished. Simulation over. Clearing rl_env...")
    agent.cleanup()

# Running Tune framework
def run_tune(config, episodes, root_dir, is_schedule):
    ...
    log.info("Tune run over. Clearing rl_env...")

# Killing Ray
def kill_ray():
    rl_utils.kill_processes_with_name("ray_")
    if rl_utils.list_processes("ray_"):
        # Show 'em who's boss
        rl_utils.kill_processes_with_name("ray_", use_sigkill=True)

# Clean all redundancy process
def clean():
    ''' A big fat hammer to get rid of all the debris left over by ray '''
    log.info("Removing all previous traces of Mininet and ray")
    kill_ray()
    os.system('sudo mn -c')
    rl_utils.kill_processes_with_name("goben")
    rl_utils.kill_processes_with_name("go_ctrl")
    rl_utils.kill_processes_with_name("node_control")
...
def get_args(args=None):
    ...
    p.add_argument('--agent',
                   ...
                   default="PG",
                   type=str.lower,
                   help='must be string of either: PPO, DDPG, PG, DCTCP, TCP_NV, PCC, or TCP')
...
# Training Ray
def train_ray(args=None):
    # Fix a bug introduced by a Google extension
    import absl.logging
    try:
    ...
    log.info("Registering the RL environment...")
    register_env("rl_env", get_env)
    ...
    rl_utils.check_dir(output_dir)
    
    # Dump the configuration
    rl_utils.dump_json(path=output_dir, name="ray_config", data=config)
    rl_utils.check_dir("/tmp/ray_plasma")
    ...
    try:
        if args.tune:
            run_tune(config, args.episodes, args.root_output, args.schedule)
        else:
    ...
    rl_utils.change_owner(args.root_output)
